# Primeros pasos: configuración inicial y la tarjeta "Puesta en marcha"

Esta guía explica qué ocurre la primera vez que abres una instalación nueva de DentalPin y cómo el panel de inicio te ayuda a terminar de configurar la clínica sin soporte externo.

## 1. Configuración inicial (`/setup`)

La primera visita a una instalación nueva abre el asistente de configuración. Lleva unos dos minutos.

1. **Cuenta de administrador** — tu nombre, correo y una contraseña (mínimo 8 caracteres con letras y números). Será la cuenta administradora de la clínica.
2. **Clínica** — nombre de la clínica, **país** e identificación fiscal. El país fija por ti la zona horaria, la moneda y el IVA (puedes cambiarlos en *Zona horaria y moneda*). Para España, el asistente comprueba el formato del NIF/CIF y avisa si la letra de control no cuadra.

Pulsa **Crear mi clínica**. DentalPin crea por ti:

- el catálogo de tratamientos y los tipos de IVA (España: exento / 10 % / 21 %; otros países: un único tipo exento editable),
- una serie de facturas (`FAC`) y otra de rectificativas (`RECT`),
- un gabinete ("Gabinete 1"),
- un horario de lunes a viernes (España: 09–14 y 16–20; resto: 09–18).

Todo se puede cambiar después en *Configuración*.

> Usa el selector de idioma de la cabecera del asistente para cambiar el idioma de la interfaz en cualquier momento.

## 2. La tarjeta "Puesta en marcha"

Tras entrar, los administradores ven al principio del panel de inicio la tarjeta **Puesta en marcha**, con una barra de progreso y los pasos pendientes:

| Paso | Qué significa | A dónde lleva |
|---|---|---|
| Datos de la clínica | Nombre, CIF/NIF y dirección que aparecen en presupuestos y facturas | *Configuración → General → Clínica* |
| Gabinetes | Al menos un gabinete para poder crear citas | Diálogo en la propia tarjeta |
| Horario de la clínica | Tus horas reales de apertura (una instalación nueva puede seguir "abierta 24 h") | Diálogo con horarios predefinidos |
| Tu equipo | Añade a los profesionales que atienden citas, o márcate como profesional si trabajas solo | Diálogo en la propia tarjeta |
| Catálogo de tratamientos | Tratamientos para presupuestar, planificar y facturar | *Configuración → Clínica → Catálogo* |
| Serie de facturas | Necesaria para emitir la primera factura | *Configuración → Facturación → Series* |
| Opcional: VeriFactu (España), Envío de emails, Primer paciente | Siguientes pasos recomendados | Módulos / Notificaciones / Pacientes |

- **Configurar** abre el paso (un diálogo si existe; si no, la pantalla de ajustes).
- **Omitir** (icono del ojo) oculta un paso que no necesitas; **Deshacer** lo recupera.
- **Ocultar** quita la tarjeta para toda la clínica; los pasos siguen resolviéndose solos según aparecen los datos.
- La tarjeta desaparece por sí sola cuando todos los pasos obligatorios están completados.

Los pasos se comprueban contra datos reales: si borras tu último gabinete, el paso vuelve a aparecer.

## 3. Modo guiado

**Modo guiado** te lleva por los pasos pendientes página a página. Una barra bajo la cabecera muestra *Paso N de M*, con **Siguiente** (vuelve a comprobar y salta al siguiente pendiente) y **Salir**. En el último paso, **Finalizar** te devuelve al inicio.

## 4. Dar de alta compañeros sin email

En *Tu equipo* (o *Configuración → Personas → Usuarios*), crea el usuario y **deja la contraseña vacía**. DentalPin muestra un **enlace de acceso** de un solo uso que puedes **copiar** o **enviar por WhatsApp**. Tu compañero lo abre, elige su contraseña y entra directamente. El enlace sirve una vez y caduca a los siete días.

El icono de enlace en cualquier fila de usuario genera un enlace nuevo, útil cuando alguien olvida su contraseña.

> Trátalo como una contraseña: quien tenga el enlace puede entrar con esa cuenta hasta que se use.
